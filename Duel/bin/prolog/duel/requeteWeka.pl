...
explore(Time,Size,Radius,MapWidth):-
	...
	areaCovered(Radius,Size,MapWidth),
	jpl_call('sma.actionsBehaviours.PrologJ48Behavior',executeExplore,[],@(void)).
...
hunt(Life,Time,OffSize,DefSize,Radius,MapWidth,EnemyInSight):-
	...
	being_attacked(Time),
	not(EnemyInSight),
	jpl_call('sma.actionsBehaviours.PrologJ48Behavior',executeHunt,[],@(void)).
...
attack(EnemyInSight):-
	EnemyInSight,
	jpl_call('sma.actionsBehaviours.PrologJ48Behavior',executeAttack,[],@(void)).


/*retreat(Life,Time):-  */
/*	not(inGoodHealth(Life)), */
/*	being_attacked(Time),  */
/*	jpl_call('sma.actionsBehaviours.PrologJ48Behavior',executeRetreat,[],@(void)). */
