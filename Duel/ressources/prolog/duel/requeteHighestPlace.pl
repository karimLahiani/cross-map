...
explore(Time,Size,Radius,MapWidth):-
	...
	areaCovered(Radius,Size,MapWidth),
	jpl_call('sma.actionsBehaviours.PrologHighestPlaceBehavior',executeExplore,[],@(void)).
...
hunt(Life,Time,OffSize,DefSize,Radius,MapWidth,EnemyInSight):-
	...
	being_attacked(Time),
	not(EnemyInSight),
	jpl_call('sma.actionsBehaviours.PrologHighestPlaceBehavior',executeHunt,[],@(void)).
...
attack(EnemyInSight):-
	EnemyInSight,
	jpl_call('sma.actionsBehaviours.PrologHighestPlaceBehavior',executeAttack,[],@(void)).


/*retreat(Life,Time):-  */
/*	not(inGoodHealth(Life)), */
/*	being_attacked(Time),  */
/*	jpl_call('sma.actionsBehaviours.PrologHighestPlaceBehavior',executeRetreat,[],@(void)). */
